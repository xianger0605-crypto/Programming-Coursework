...
void take_money();
void send_money();
void check_account_balance();  // NEW FUNCTION
...
int main() {
    ...
    do {
        ...
        switch(user_selection) {
            ...
            case 6: check_account_balance(); break;  // NEW OPTION
...
// NEW FUNCTION: Check Account Balance
void check_account_balance() {
    printf("\n=== Check Account Balance ===\n");
    printf("View Your Account Information\n");
    printf("-----------------------------\n");
    ...
    // Create file path for account
    char file_path[100];
    sprintf(file_path, "database/acc_%d.txt", account_number);
    
    // Try to open account file
    FILE* account_file = fopen(file_path, "r");
    if (account_file == NULL) {
        printf("------------------------------------------------\n");
        printf("ERROR: Account not found in our system.\n");
    ...
    char stored_security_code[10] = "";
    int stored_account_number = 0;
    float account_balance = 0.0;
    
    char file_line[200];
    int information_found = 0;
    
    // Read account information from file
    while (fgets(file_line, sizeof(file_line), account_file)) {
        // Remove newline character
        file_line[strcspn(file_line, "\n")] = 0;
        
        // Extract customer name
        if (strstr(file_line, "Customer:") != NULL) {
            sscanf(file_line, "Customer: %99[^\n]", customer_name);
            information_found = 1;
        }
        // Extract ID number
        else if (strstr(file_line, "ID_Number:") != NULL) {
            sscanf(file_line, "ID_Number: %49[^\n]", customer_id);
        }
        // Extract account type
        else if (strstr(file_line, "Account_Type:") != NULL) {
            sscanf(file_line, "Account_Type: %19[^\n]", account_type);
        }
        // Extract security code
        else if (strstr(file_line, "Security_Code:") != NULL) {
            sscanf(file_line, "Security_Code: %9[^\n]", stored_security_code);
        }
        // Extract account number
        else if (strstr(file_line, "Account_No:") != NULL) {
            sscanf(file_line, "Account_No: %d", &stored_account_number);
        }
        // Extract account balance
        else if (strstr(file_line, "Available_Balance:") != NULL) {
            sscanf(file_line, "Available_Balance: %f", &account_balance);
        }
    }
    ...
    if (!information_found) {
        printf("------------------------------------------------\n");
        printf("ERROR: Could not read account information.\n");
        printf("The account file may be corrupted.\n");
        printf("------------------------------------------------\n");
        while (getchar() != '\n');
        return;
    }
    ...
    if (strcmp(security_code, stored_security_code) != 0) {
        printf("------------------------------------------------\n");
        printf("SECURITY ALERT: Invalid security code!\n");
        printf("Access to account information denied.\n");
        printf("Please contact bank customer service.\n");
        printf("------------------------------------------------\n");
        while (getchar() != '\n');
        return;
    }
    
    // Display account information in a nice format
    printf("\n");
    printf("================================================\n");
    printf("            ACCOUNT INFORMATION\n");
    printf("================================================\n");
    printf(" Account Holder : %-29s \n", customer_name);
    printf(" Account Number : %-29d \n", stored_account_number);
    printf(" ID Number      : %-29s \n", customer_id);
    printf(" Account Type   : %-29s \n", account_type);
    printf("                                              \n");
    ...
    if (account_balance == 0.0) {
        printf("NOTE: Your account balance is RM 0.00\n");
        printf("      Consider making a deposit to start using your account.\n");
    }
    else if (account_balance < 100.0) {
        printf("NOTE: Your balance is below RM 100.00\n");
        printf("      Maintain minimum balance to avoid service charges.\n");
    }
    else if (account_balance > 10000.0) {
        printf("NOTE: You have a healthy account balance!\n");
        printf("      Consider our premium savings plans for better returns.\n");
    }
    
    // Record this activity in log
    save_to_log("BALANCE_CHECKED");
